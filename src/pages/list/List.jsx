...
const List = () => {
  ...
  //handle data coming from Header component
  const location = useLocation()

  //storing the incoming state(data) from header component to respective variables
  const [destination, setDestination] = useState(location.state.destination)
  const [dates, setDates] = useState(location.state.dates)
  const [options, setOptions] = useState(location.state.options)

  //date selection component hide/unhide
  const [openDate, setOpenDate] = useState(false)


  //to handle minPrice & maxPrice
  const [min, setMin] = useState(undefined)
  const [max, setMax] = useState(undefined)


    //get data from api using our custom hook
